...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        self.loss.func = PearsonCorrelationLoss()
...
if __name__ == '__main__':
    ...
    if args.kfold==True:
        ...
        for k in [0,1,2,5]:
            model = torch.load(f'model_{k}.pt')
        ...
        # model = pl.LightningModule.load_from_checkpoint('./lightning_logs/version_126/checkpoints/epoch=0-step=125.ckpt')
        model = torch.load('model.pt')
        trainer = pl.Trainer(accelerator="gpu", devices=1, max_epochs=args.max_epoch, log_every_n_steps=1)
        predictions = trainer.predict(model=model, datamodule=dataloader)

        predictions_result = list(round(float(i), 1) for i in torch.cat(predictions))
