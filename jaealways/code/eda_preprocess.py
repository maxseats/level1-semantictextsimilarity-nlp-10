...
if __name__ == '__main__':
    df1 = pd.read_csv('../data/train_aug_eda_autolabel.csv')
    df2 = pd.read_csv('../data/stopword_space_Addlabel5.csv', encoding='UTF8')

    test=df1['label'].tolist()
    test
    df=pd.concat([df1,df2.iloc[9324:]])
    df.index = [x for x in range(df.shape[0])]

    df.to_csv('../data/train_aug_eda_autolabel.csv')
